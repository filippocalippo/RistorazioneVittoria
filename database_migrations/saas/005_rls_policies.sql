...
CREATE POLICY "Anyone can view business rules" ON business_rules FOR SELECT TO anon, authenticated USING (true);
DROP POLICY IF EXISTS "Managers can manage business rules" ON business_rules;
CREATE POLICY "Managers can manage business rules" ON business_rules FOR ALL TO authenticated USING (is_manager()) WITH CHECK (is_manager());

-- DELIVERY_CONFIGURATION
DROP POLICY IF EXISTS "Anyone can view delivery config" ON delivery_configuration;
CREATE POLICY "Anyone can view delivery config" ON delivery_configuration FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "Managers can manage delivery config" ON delivery_configuration;
CREATE POLICY "Managers can manage delivery config" ON delivery_configuration FOR ALL TO authenticated USING (is_manager()) WITH CHECK (is_manager());

-- ORDER_MANAGEMENT
DROP POLICY IF EXISTS "Staff can view order management" ON order_management;
CREATE POLICY "Staff can view order management" ON order_management FOR SELECT TO authenticated USING (is_staff());
DROP POLICY IF EXISTS "Managers can manage order settings" ON order_management;
CREATE POLICY "Managers can manage order settings" ON order_management FOR ALL TO authenticated USING (is_manager()) WITH CHECK (is_manager());

-- KITCHEN_MANAGEMENT
DROP POLICY IF EXISTS "Staff can view kitchen settings" ON kitchen_management;
CREATE POLICY "Staff can view kitchen settings" ON kitchen_management FOR SELECT TO authenticated USING (is_staff());
DROP POLICY IF EXISTS "Managers can manage kitchen settings" ON kitchen_management;
CREATE POLICY "Managers can manage kitchen settings" ON kitchen_management FOR ALL TO authenticated USING (is_manager()) WITH CHECK (is_manager());

-- DISPLAY_BRANDING
DROP POLICY IF EXISTS "Anyone can view branding" ON display_branding;
CREATE POLICY "Anyone can view branding" ON display_branding FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "Managers can manage branding" ON display_branding;
CREATE POLICY "Managers can manage branding" ON display_branding FOR ALL TO authenticated USING (is_manager()) WITH CHECK (is_manager());

-- DASHBOARD_SECURITY
DROP POLICY IF EXISTS "Managers can manage dashboard security" ON dashboard_security;
CREATE POLICY "Managers can manage dashboard security" ON dashboard_security FOR ALL TO authenticated USING (is_manager()) WITH CHECK (is_manager());
...
USING ((attivo = true AND (data_inizio IS NULL OR data_inizio <= now()) AND (data_fine IS NULL OR data_fine >= now())) OR is_manager());
DROP POLICY IF EXISTS "Managers can manage banners" ON promotional_banners;
CREATE POLICY "Managers can manage banners" ON promotional_banners FOR ALL TO authenticated USING (is_manager()) WITH CHECK (is_manager());

-- INGREDIENT_CONSUMPTION_RULES
DROP POLICY IF EXISTS "Staff can view consumption rules" ON ingredient_consumption_rules;
CREATE POLICY "Staff can view consumption rules" ON ingredient_consumption_rules FOR SELECT TO authenticated USING (is_staff());
DROP POLICY IF EXISTS "Managers can manage consumption rules" ON ingredient_consumption_rules;
CREATE POLICY "Managers can manage consumption rules" ON ingredient_consumption_rules FOR ALL TO authenticated USING (is_manager()) WITH CHECK (is_manager());

-- INVENTORY_LOGS
DROP POLICY IF EXISTS "Staff can view inventory logs" ON inventory_logs;
CREATE POLICY "Staff can view inventory logs" ON inventory_logs FOR SELECT TO authenticated USING (is_staff());
DROP POLICY IF EXISTS "Staff can insert inventory logs" ON inventory_logs;
CREATE POLICY "Staff can insert inventory logs" ON inventory_logs FOR INSERT TO authenticated WITH CHECK (is_staff());
...
USING (EXISTS (SELECT 1 FROM ordini o WHERE o.id = order_id AND o.cliente_id = auth.uid()));
DROP POLICY IF EXISTS "Staff can view all payments" ON payment_transactions;
CREATE POLICY "Staff can view all payments" ON payment_transactions FOR SELECT TO authenticated USING (is_staff());
DROP POLICY IF EXISTS "System can create payments" ON payment_transactions;
CREATE POLICY "System can create payments" ON payment_transactions FOR INSERT TO authenticated WITH CHECK (true);

-- STATISTICHE_GIORNALIERE
DROP POLICY IF EXISTS "Staff can view statistics" ON statistiche_giornaliere;
CREATE POLICY "Staff can view statistics" ON statistiche_giornaliere FOR SELECT TO authenticated USING (is_staff());
DROP POLICY IF EXISTS "Managers can manage statistics" ON statistiche_giornaliere;
CREATE POLICY "Managers can manage statistics" ON statistiche_giornaliere FOR ALL TO authenticated USING (is_manager()) WITH CHECK (is_manager());

-- INVENTORY FUNCTIONS

CREATE OR REPLACE FUNCTION set_ingredient_stock(p_ingredient_id UUID, p_quantity NUMERIC, p_note TEXT DEFAULT NULL)
RETURNS VOID LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE v_old_qty NUMERIC;
BEGIN
    SELECT stock_quantity INTO v_old_qty FROM ingredients WHERE id = p_ingredient_id FOR UPDATE;
    UPDATE ingredients SET stock_quantity = p_quantity WHERE id = p_ingredient_id;
    INSERT INTO inventory_logs (ingredient_id, tipo, quantita, quantita_precedente, quantita_nuova, note, created_by)
    VALUES (p_ingredient_id, 'adjust', p_quantity - COALESCE(v_old_qty, 0), v_old_qty, p_quantity, p_note, auth.uid());
END; $$;

CREATE OR REPLACE FUNCTION adjust_ingredient_stock(p_ingredient_id UUID, p_delta NUMERIC, p_tipo TEXT DEFAULT 'adjust', p_ordine_id UUID DEFAULT NULL, p_note TEXT DEFAULT NULL)
RETURNS VOID LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE v_old_qty NUMERIC; v_new_qty NUMERIC;
BEGIN
    SELECT stock_quantity INTO v_old_qty FROM ingredients WHERE id = p_ingredient_id FOR UPDATE;
    v_new_qty := COALESCE(v_old_qty, 0) + p_delta;
    UPDATE ingredients SET stock_quantity = v_new_qty WHERE id = p_ingredient_id;
    INSERT INTO inventory_logs (ingredient_id, ordine_id, tipo, quantita, quantita_precedente, quantita_nuova, note, created_by)
    VALUES (p_ingredient_id, p_ordine_id, p_tipo, ABS(p_delta), v_old_qty, v_new_qty, p_note, auth.uid());
END; $$;

CREATE OR REPLACE FUNCTION update_ingredient_stock(p_ingredient_id UUID, p_delta NUMERIC, p_note TEXT DEFAULT NULL)
RETURNS VOID LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN PERFORM adjust_ingredient_stock(p_ingredient_id, p_delta, 'adjust', NULL, p_note); END; $$;

-- BANNER ANALYTICS FUNCTIONS

CREATE OR REPLACE FUNCTION increment_banner_view(p_banner_id UUID)
RETURNS VOID LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN UPDATE promotional_banners SET impressions = impressions + 1 WHERE id = p_banner_id; END; $$;

CREATE OR REPLACE FUNCTION increment_banner_click(p_banner_id UUID)
RETURNS VOID LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN UPDATE promotional_banners SET clicks = clicks + 1 WHERE id = p_banner_id; END; $$;
...
BEGIN
    RETURN QUERY SELECT pb.impressions, pb.clicks, CASE WHEN pb.impressions > 0 THEN (pb.clicks::NUMERIC / pb.impressions * 100) ELSE 0 END FROM promotional_banners pb WHERE pb.id = p_banner_id;
END; $$;
